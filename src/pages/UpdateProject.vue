...
import { getDocs, doc, collection, query, updateDoc, arrayUnion, arrayRemove, deleteDoc } from 'firebase/firestore'
...
const checkForm = () => {
    ...
    if (projectDetails.name && projectDetails.client
        ...
        if (projectDetails.images.length < 1)
            checkDataIsNotEmpty.images = false;
        if (projectDetails.is360 && !projectDetails.link360)
...
const onchange = async () => {
    ...
    for (let i = 0; i < file.value.files.length; i++) {
        ...
        uploadToFirebase(compressedImage, image.name);
    }
}
...
const uploadToFirebase = async (optimizedImageBuffer: any, imageName: string) => {
    ...
    projectDetails.images.push(url);
}

const saveDeleteImgIndex = ref<number[]>([]);
const deleteImgFromDB = async () => {
    const allProjects = await getDocs(collection(db, 'projects'));
    saveDeleteImgIndex.value.forEach((index) => {
        allProjects.forEach((project) => {
            if (projectId.value === project.id) {
                const imgTarget = project.data().images[index];
                try {
                    updateDoc(project.ref, {
                        images: arrayRemove(imgTarget),
                    })
                } catch (error) {
                    console.log("Remove project fail!! " + error)
                }
            }
        })
    })
...
const deleteImg = (index: number) => {
    projectDetails.images.splice(index, 1);
    saveDeleteImgIndex.value.push(index);
}
...
<template>
    ...
    <div v-else class="">
        <div class="border-b border-solid pb-4 mb-6">
            <h1 class="text-3xl font-bold">{{ projectDetails.name }}</h1>
        </div>
        <form class=" w-full">
            <div class="grid grid-cols-2 gap-8 mb-8">
                <div class="col-span-1">
                    <div class=" shadowBox mb-8 bg-white p-6 rounded-md border border-solid">
                        <div class="w-full mb-8 min-w-[200px]">
                            ...
                            <div class="relative flex items-center h-12">
                                <input v-model="projectDetails.name" type="text" id="project-name"
                        ...
                        <div class="flex gap-4 mb-5">
                            <div class="w-full max-w-sm min-w-[200px]">
                                ...
                                <div class="relative mt-2">
                                    <select v-model="projectDetails.type" id="project-type"
                        ...
                        <div v-if="projectDetails.is360" class="flex items-center mt-5 gap-2 text-[10px]">
                        ...
                        <div class="w-full my-8 min-w-[200px]">
                            ...
                            <div class="relative flex items-center h-12">
                                <input v-model="projectDetails.year" type="number" id="project-year"
                                    class="mt-2 w-full h-12 pl-3 pr-10 py-2 bg-transparent placeholder:text-slate-400 text-slate-600 text-sm border border-slate-200 rounded-md transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow"
                                    placeholder="Type here..." />
                        ...
                        <div class="w-full mb-8 min-w-[200px]">
                            ...
                            <div class="relative flex items-center h-12">
                                <input type="text" id="project-client"
                                    class="mt-2 w-full h-12 pl-3 pr-10 py-2 bg-transparent placeholder:text-slate-400 text-slate-600 text-sm border border-slate-200 rounded-md transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow"
                                    placeholder="Type here..." v-model="projectDetails.client" />
                    ...
                    <div class="w-full bg-white p-6 rounded-md border border-solid">
                        <label for="project-name">Mô tả của dự án</label>
                        <div class="relative flex items-center">
                            <textarea spellcheck="false" v-model="projectDetails.description"
                                placeholder="Nhập mô tả cho dự án của bạn..." cols="1" rows="10" name="" id=""
                ...
                <div class="col-span-1  shadowBox bg-white p-6 rounded-md border border-solid">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div class="relative img-box" v-for="(img, index) in projectDetails.images">
                            <img :src="img" alt="">
                            <div
